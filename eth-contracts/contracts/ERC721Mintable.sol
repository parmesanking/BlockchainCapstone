...
modifier whenNotPaused() {
        require(_paused == false, "token is paused");
        _;
    }

    modifier paused() {
        require(_paused == true, "token isn't paused");
        _;
    }
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        require(to != _tokenOwner[tokenId] , "the given address already owns the tokenId") ;
        require(msg.sender == _tokenOwner[tokenId] || isApprovedForAll(_tokenOwner[tokenId], msg.sender) , "requires the msg sender to be the owner of the contract or it requires isApprovedForAll");
...
contract AGOContract is ERC721Metadata ("AGOContract", "AC","https://s3-us-west-2.amazonaws.com/udacity-blockchain/capstone/"){
